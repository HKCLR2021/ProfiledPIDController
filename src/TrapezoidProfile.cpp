...
TrapezoidProfile::Calculate(Duration dt, State goal, State current) {
  ...
  // spdlog::debug("current {} {}", result.position , result.velocity );
  spdlog::debug("m_timeFromStart {:0.2f} >>> m_endAccel {:0.2f} | m_endFullSpeed {:0.2f} | m_endDeccel {:0.2f}", 
  ...
  if (dt < m_endAccel) { // acceleration stage
    ...
    spdlog::debug("<acc stage> pos inc [{:0.2f}], vel inc[{:0.2f}mm/s]", (m_current.velocity + result.velocity) / 2.0 * dt, dt * m_constraints.maxAcceleration );
    ...
    spdlog::debug("<fullspeed stage> pos inc [{:0.2f}]", (m_current.velocity +
                        m_endAccel * m_constraints.maxAcceleration / 2.0) *
                           m_endAccel +
                       m_constraints.maxVelocity * (dt - m_endAccel));
    ...
    result.position =
        ...
        (goal.velocity + timeLeft * m_constraints.maxAcceleration / 2.0) *
            timeLeft;
    spdlog::debug("dec stage" );

  } else {
    result = goal;
  }
  // spdlog::warn("result {} {}", result.position , result.velocity );
  spdlog::info("<TrapezoidProfile> output: dt [{:0.2f}] pos[{:0.2f}/{:0.2f}], vel[{:0.2f}/{:0.2f} mm/s]", 
